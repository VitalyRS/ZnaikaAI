...
def generate_prompt(text, url, style_key, length_key):
    ...
    "style1": (
        ...
        f"В самом тексте не должно быть ни одной звёздочки, даже если она была в оригинале. "
    ...
    "style2": (
        ...
        f"В теле текста не должно быть ни одной *. Не используй восклицательные знаки. "
        f"Длина около {length} символов. В конце — ссылка: {url}, 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style3": (
        f"Сделай информативный пост в инфостиле: чётко, по делу, с абзацами. "
        f"Заголовок выдели жирным через **двойные звёздочки**, а в теле текста никаких * и восклицаний. "
        f"Длина — около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style4": (
        f"Сделай пост в формате сторителлинга: сначала заголовок, потом короткий рассказ. "
        f"Заголовок выдели как **жирный**. В остальной части текста не должно быть звёздочек и восклицаний. "
        f"Объём около {length} символов. В конце добавь ссылку: {url}, 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style5": (
        f"Сделай научно-популярный пересказ на русском — понятно, но не примитивно. "
        f"Заголовок выдели как **жирный**. Не допускай ни одной звёздочки в теле текста, даже если они есть в оригинале. "
    ...
    "style6": (
        f"Сделай пересказ в стиле падонков — язвительно, иронично, местами абсурдно. "
        f"Заголовок выдели жирным (**двойные звёздочки**), но в теле текста никаких *. "
        f"Объём около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style7": (
        f"Сделай восторженный пост в стиле: 'как всё хорошо, какие молодцы'. "
        f"Заголовок выдели жирным через **двойные звёздочки**, в остальном — никаких * и восклицаний. "
        f"Длина около {length} символов. В конце ссылка: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),
}
