...
def generate_prompt(text, url, style_key, length_key):
    ...
    "style1": (
        ...
        f"Только заголовок выдели жирным текстом для телеграм канала, в остальном — без оформления и восклицательных знаков. "
        f"Длина около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Исходный текст:\n{text}"
    ),

    "style2": (
        f"Сделай ироничный пересказ текста на русском. Заголовок выдели выдели жирным текстом для телеграм канала, без восклицаний и другого оформления. "
        f"Длина около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style3": (
        f"Сделай информативный пост на русском в инфостиле: чётко, по делу, с абзацами. "
        f"Заголовок выдели выдели жирным текстом для телеграм канала, больше ничего не выделяй. Объём — примерно {length} символов. "
        f"В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style4": (
        f"Сделай пост в формате сторителлинга на русском. Сначала заголовок, затем рассказ в форме истории. "
        f"Выдели только заголовок выдели жирным текстом для телеграм канала, не используй символы оформления или восклицания. "
        f"Объём — около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style5": (
        f"Сделай научно-популярный пересказ на русском. Объясни понятно, без примитивизации. "
        f"Стиль спокойный. Только заголовок выдели жирным текстом для телеграм канала, остальное — без оформления и восклицаний. "
        f"Длина около {length} символов. В конце добавь ссылку: {url}, 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style6": (
        f"Сделай пересказ в стиле падонков на русском — с иронией и абсурдом, смешно. "
        f"Выдели только заголовок как **ОГЛАВЛЕНИЕ**, больше ничего не оформляй и не используй восклицания. "
        f"Длина — около {length} символов. В конце добавь ссылку: {url}, затем 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),

    "style7": (
        f"Сделай восторженный пересказ в духе: 'как всё хорошо, как заботятся о народе'. "
        f"Выдели только заголовок  жирным текстом для телеграм канала, не используй символы оформления и восклицания. "
        f"Длина около {length} символов. В конце добавь ссылку: {url}, 5 испанских и 5 русских хэштегов. "
        f"Текст:\n{text}"
    ),
}
